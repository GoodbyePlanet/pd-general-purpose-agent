...
I'm a FastAPI app connected to Slack via Socket Mode (WebSocket). I use a two-tier multi-agent setup built with LangGraph — a *Root Agent* that handles conversations and can delegate to a *Web Research sub-agent* equipped with web search (Tavily) and URL fetching tools, both powered by Claude (Anthropic) in a ReAct loop. I run as a single Docker container on a Hetzner VPS with one worker (to avoid duplicate Slack events). For the full picture with Mermaid diagrams, check out `architecture.md` in the project repo, link https://github.com/ProductDock/pidi/blob/main/architecture.md.
